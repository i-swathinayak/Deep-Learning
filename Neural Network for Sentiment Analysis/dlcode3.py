...
def FirstLayer(net, l2_reg_val, is_training):
    ...
    net = tf.contrib.layers.fully_connected(net, 40, activation_fn=None, biases_initializer=None)
...
def EmbeddingL2RegularizationUpdate(embedding_variable, net_input, learn_rate, l2_reg_val):
    ...
    #net = tf.matmul(embedding_variable, net_input)
    
    net = tf.nn.l2_normalize(net_input, axis=1)
    grad_comp1 = tf.transpose(tf.matmul(net, embedding_variable))
    grad_comp2 = tf.transpose(tf.matmul(grad_comp1, net))
    gradient = 2 * l2_reg_val * grad_comp2
...
def EmbeddingL1RegularizationUpdate(embedding_variable, net_input, learn_rate, l1_reg_val):
    ...
    # TODO(student): Change this to something useful. Currently, this is a no-op.
    net = tf.nn.l2_normalize(net_input, axis=1)
    grad_comp  = tf.matmul(net, embedding_variable)
    grad_comp1 = tf.math.sign(grad_comp)
    grad_comp2 = tf.matmul(tf.transpose(net), grad_comp1)
    gradient = l1_reg_val * grad_comp2
    return embedding_variable.assign(embedding_variable - (learn_rate * gradient))
...
def SparseDropout(slice_x, keep_prob=0.5):
    ...
    nzrow, nzcol = np.nonzero(slice_x)
    idx = np.random.choice(len(nzrow), int(np.floor((1-keep_prob) * len(nzrow))), replace=False)
    slice_x[nzrow[idx], nzcol[idx]] = 0
    return slice_x
...
def ComputeTSNE(embedding_matrix):
    ...
    """
    embed_matrix = sklearn.manifold.TSNE(n_components=2).fit_transform(embedding_matrix)
    return embed_matrix[:, :]
...
def VisualizeTSNE(sess):
    ...
    # assign colors for 4 different classes
    class_color = {'positive' : 'blue', 'negative' : 'orange', 'location' : 'green', 'furniture': 'red'}

    for key in class_to_words:
        word_indices=[]
        for val in class_to_words[key]:
            # retrieve the corresponding word indices from TERM_INDEX dictionary and store in the list
            word_indices.append(TERM_INDEX[val])

        # splice the tsne embedding numpy array on the retrieved word indices
        arr_indx = tsne_embeddings[word_indices, :]

        # assign the two dimensions of arr_indx to x and y coordinates of the plot
        x = arr_indx[:, 0]
        y = arr_indx[:, 1]
        ax.scatter(x, y, color=class_color[key])
        # label the coordinates with their corresponding words
        for i, txt in enumerate(class_to_words[key]):
            #label = txt + '(' + str(x[i]) + ',' + str(y[i]) + ')'
            ax.annotate(txt, (x[i], y[i]), textcoords="offset points", xytext=(0, 4), ha='center', size=6)

    fig.savefig('tsne_embeds.pdf')
