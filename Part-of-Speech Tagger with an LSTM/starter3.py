...
#
    ...
    def BuildMatrices(dataset):
        ...
        lengths = numpy.zeros(N, dtype=numpy.int64)
        for m in range(0, N):
            temp = len(dataset[m])
            terms = [i[0] for i in dataset[m]]
            tags = [j[1] for j in dataset[m]]
            terms_matrix[m, 0:temp] = terms
            tags_matrix[m, 0:temp] = tags
            lengths[m] = temp

        matrices_list = (terms_matrix, tags_matrix, lengths)
...
class SequenceModel(object):
    ...
    def build_inference(self):
        ...
        embedding_var = tf.get_variable('embedding', shape=[self.num_terms, state_size])
    ...
    def build_training(self):
        ...
        Hint:
            ...
            - tf.losses.get_total_loss() should return a valid tensor (without raising
                ...
                non-empty list.
        """
        self.lengths_mask = self.lengths_vector_to_binary_matrix(self.lengths)
        self.y = tf.placeholder(tf.int64, [None, self.max_length], name='y')
        tf.losses.add_loss(tf.contrib.seq2seq.sequence_loss(self.logits, self.y, self.lengths_mask))
    ...
    def train_epoch(self, terms, tags, lengths, batch_size=32, learn_rate=0.007):
        ...
        for si in range(0, terms.shape[0], batch_size):
            ...
            self.sess.run(self.train_op, { self.x: slice_x, self.y: tags[indices[si:se]], self.learning_rate: learn_rate, self.lengths: lengths[indices[si:se]] })
